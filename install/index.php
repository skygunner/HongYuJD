        ...
        if(isset($_POST['install'])){
            if(($_POST['db_host']) and ($_POST['db_name']) and ($_POST['db_pass']) !== ''){
                ...
                if (!@$link = mysql_connect($db_host, $db_user, $db_pass)) { //检查数据库连接情况
                    ...
                    $config_str .= "\n\n";
                    $config_str .= "define('API_TIME', '2016-05-18 18:18:58');";
                    $config_str .= "\n\n";
                    $config_str .= "?>";
                    $ff = fopen($files,"w+");
                    ...
                    $config_mobile .= "\n\n";
                    $config_mobile .= "define('API_TIME', '2016-05-18 18:18:58');";
                    $config_mobile .= "\n\n";
                    $config_mobile .= '?>';
                    include ('install.php');
                    exit;
...
<div style="background: url(login_dl.jpg) no-repeat;background-size:100% 100%;width: 100%;height: 100%;float: left;position:fixed;">
    <h1 style="text-align: center;padding: 80px 0 30px 0;border: ">HongYu-V6.0 多用户商城系统</h1>
